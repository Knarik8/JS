...
const map = {
    ...
    render(snakePointsArray, foodPoint, barrierPoint) {
        ...
        this.usedCells = [];
        this.score = document.querySelector('.score_text'); // ссылка на объект ДОМ
        this.myScore = null,
            ...
            snakePointsArray.forEach((point, idx) => {
                ...
                this.score.textContent = `Счет: ${this.myScore++}`;
            });
...
const barrier = {
    ...
    // чтобы узнать попадает ли змейка на препятствие
...
const game = {
    ...
    getRandomFreeCoordinates() {
        const exclude = [this.food.getCoordinates(), ...this.snake.getBody()]; //занятые ячейки змеей и едой


        while (true) {
            const rndPoint = { //объект с рандомными координатами
                x: Math.floor(Math.random() * this.config.getColsCount()),
                y: Math.floor(Math.random() * this.config.getRowsCount()),
            };
            // сравниваем точки, если наша сгенерированная точка не попадает в занятую ячейку
